...
const buildOuterCircle = (config) => {
  ...
  circleWaveCanvasContext.fillStyle = outerCircle.fillStyle;
  circleWaveCanvasContext.fill();
};

const buildRotateCircle = (config, rotateAngle) => {
  ...
  circleWaveCanvasContext.lineWidth = rotateCircle.lineWidth;
  circleWaveCanvasContext.strokeStyle = rotateCircle.strokeStyle;
  circleWaveCanvasContext.stroke();

  rotateCircle.rotate++;
  circleWaveCanvasContext.setLineDash([1, 110]);
  circleWaveCanvasContext.setTransform(1, 0, 0, 1, 0, 0);
};
  ...
  init() {
    ...
    for (let i = 0; i < this.totalPoints; i++) {
      const point = new Point(
        this.index + i,
        this.pointGap * i, // Canvas 내에서 Point 객체의 첫 시작 위치와 Point 객체 간 간격을 조절하는 부분입니다.
...
class App {
  ...
  animate(time) {
    ...
    buildRotateCircle(
      circleWaveConfig,
      (circleWaveConfig.rotateCircle.rotate * Math.PI) / 360
    );
    requestAnimationFrame(this.animate.bind(this));
  }
}
